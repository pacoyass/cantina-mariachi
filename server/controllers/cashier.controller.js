...
export const getOrdersAwaitingPayment = async (req, res) => {
  try {
    const orders = await databaseService.getOrdersByStatus(['AWAITING_PAYMENT', 'READY']);
    ...
    LoggerService.logError('Failed to get orders awaiting payment', error.stack, {
...
export const processPayment = async (req, res) => {
  try {
    ...
    if (!orderId || !paymentMethod || !amount) {
      return createError(res, 400, 'Order ID, payment method, and amount are required', 'VALIDATION_ERROR');
    }
    ...
    LoggerService.logActivity(req.user?.userId, 'PAYMENT_PROCESSED', `Processed payment for order ${orderId}`, {
      orderId,
      paymentMethod,
      amount,
      cashierId: req.user?.userId
    ...
    return createResponse(res, 200, 'Payment processed successfully', updatedOrder);
  } catch (error) {
...
export const getTodayTransactions = async (req, res) => {
  try {
    ...
    const transactions = await databaseService.getCashTransactionsByDateRange(today, tomorrow);
    const orders = await databaseService.getOrdersByDateRange(today, tomorrow, ['COMPLETED']);
    
    // Calculate stats
    const cashTotal = transactions.reduce((sum, t) => sum + t.amount, 0);
    const cardOrders = orders.filter(o => !transactions.find(t => t.orderId === o.id));
    ...
    return createError(res, 500, 'Failed to fetch transactions', 'SERVER_ERROR');
  }
};
...
export const endShiftReport = async (req, res) => {
  try {
    ...
    LoggerService.logActivity(req.user?.userId, 'SHIFT_END', 'Generated end shift report', {
      userId: req.user?.userId,
      report
    });
