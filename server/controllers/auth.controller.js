...
export const cleanupExpiredAuthData = async () => {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      await LoggerService.logSystemEvent("auth_cleanup", "ATTEMPT_CLEANUP", {
        instanceId,
      ...
      if (!locked) {
        await LoggerService.logAudit(null, "AUTH_CLEANUP_SKIPPED", null, {
          reason: "Lock acquisition failed",
          instanceId,
        });
        await LoggerService.logCronRun("auth_cleanup", "SKIPPED", {
          reason: "Lock acquisition failed",
          instanceId,
        });
        return;
      ...
      try {
        const cutoffDate = toZonedTime(
          subDays(new Date(), AUTH_DATA_RETENTION_DAYS),
          "Europe/London"
        );
        ...
        totalDeleted =
          deletedRefreshTokens.count + deletedBlacklistedTokens.count;

        // ✅ No expired tokens case
        if (totalDeleted === 0) {
          ...
          await LoggerService.logNotification(
            ...
            "WEBHOOK",
            "auth_cleanup",
            "No authentication data deleted",
            "SENT"
          );
        ...
        await LoggerService.logNotification(
          ...
          "WEBHOOK",
          "auth_cleanup",
          `Deleted ${totalDeleted} auth records`,
          "SENT"
        );
        await sendWebhook("AUTH_CLEANUP_SUCCESS", {
          deletedCount: totalDeleted,
        ...
        await LoggerService.logError("Auth cleanup failed", error.stack, {
          instanceId,
          error: error.message,
        });
        await LoggerService.logCronRun("auth_cleanup", "FAILED", {
          error: error.message,
          instanceId,
    ...
    await LoggerService.logAudit(null, "AUTH_CLEANUP_FAILED", null, {
    ...
    await sendWebhook("AUTH_CLEANUP_FAILED", {
      reason: error.message,
      instanceId,
      timestamp: toZonedTime(new Date(), "Europe/London").toISOString(),
    });
...
export const register = async (req, res) => {
  try {
    ...
    await LoggerService.logAudit(user.id, "USER_REGISTERED", user.id, {
      email,
      role,
      name,
      phone,
    ...
    await LoggerService.logError("Registration failed", error.stack, {
      email: req.body.email,
    });
    await LoggerService.logAudit(null, "USER_REGISTER_FAILED", null, {
      reason: error.message,
      email: req.body.email,
    });
...
export const logout = async (req, res) => {
  try {
   ...
   console.log("logout backend result...",result);
    ...
    // ✅ Log and audit
    if (verified && userId) {
      await LoggerService.logAudit(userId, "USER_LOGOUT_SUCCESS", userId, {
        refreshTokensCleared: refreshDeleted,
        ip: req.ip,
        userAgent: req.get("User-Agent"),
      });
      ...
      await LoggerService.logNotification(
        ...
        "WEBHOOK",
        "user_logout",
        `User ${req.user?.email || userId} logged out`,
        "SENT"
      );

      await sendWebhook("USER_LOGOUT", {
        userId,
        email: req.user?.email || null,
      });
    } else {
      // Token was invalid but we still handled it gracefully
      await LoggerService.logAudit(null, "USER_LOGOUT_FALLBACK", null, {
        reason: "Invalid or expired token, fallback logout applied",
        ip: req.ip,
        userAgent: req.get("User-Agent"),
      });
    }
    ...
    return createResponse(
      ...
      "logoutSuccess",
      {
        userMessage: verified
          ...
          : "Your session was invalid or expired, but you have been logged out safely.",
        nextSteps: "Log in again to access your account.",
      },
      req,
      {},
      "auth"
    ...
    await LoggerService.logError("Logout failed", error.stack, {
      userId: req.user?.id,
      ip: req.ip,
    });
    await LoggerService.logAudit(null, "USER_LOGOUT_FAILED", null, {
    ...
    return createError(res, 500, "Failed to log out", "SERVER_ERROR", {
      userMessage: "An unexpected error occurred while logging out.",
...
export const getToken = async (req, res) => {
  try {
    ...
    const tokenHash = await hashToken(incoming);
    const blacklisted = await databaseService.findBlacklistedToken(tokenHash);
    if (blacklisted) {
      await LoggerService.logAudit(null, "TOKEN_REVOKED", payload.userId, {
        reason: "Token is revoked.",
      });
      return createError(
        res,
        401,
        "Token is revoked. Please log in again.",
        "UNAUTHORIZED",
        {
    ...
    // Log success
    await LoggerService.logAudit(user.id, "TOKEN_VALIDATE_SUCCESS", user.id, {
      email: user.email,
    });
    await LoggerService.logNotification(
      user.id,
      "WEBHOOK",
      "token_validated",
      `Token validated for ${user.email}`,
      "SENT"
    );
    await sendWebhook("TOKEN_VALIDATED", {
      userId: user.id,
      email: user.email,
      phone: user.phone,
    });
    ...
    await LoggerService.logError("Token validation failed", error.stack, {
      userId: req.user?.id,
    });
    await LoggerService.logAudit(null, "TOKEN_VALIDATE_FAILED", null, {
      reason: error.message,
...
export const listSessions = async (req, res) => {
  ...
  try {
    ...
    const tokens = await databaseService.listRefreshTokensByUser(
      req.user.userId,
      { page, pageSize }
    );
    ...
    await LoggerService.logError("listSessions failed", error.stack, {
...
export const logoutAllSessions = async (req, res) => {
  try {
    ...
    const count = await databaseService.deleteAllRefreshTokensForUser(
      req.user.userId
    );
    res.clearCookie("accessToken", { path: "/", httpOnly: true });
    res.clearCookie("refreshToken", { path: "/", httpOnly: true });
    ...
    await LoggerService.logError("logoutAllSessions failed", error.stack, {
      userId: req.user?.userId,
    });
    return createError(
      res,
...
export const logoutOtherSessions = async (req, res) => {
  try {
    ...
    const count = await databaseService.deleteOtherRefreshTokensForUser(
      req.user.userId,
      keepHash
    );
    ...
    await LoggerService.logError("logoutOtherSessions failed", error.stack, {
      userId: req.user?.userId,
    });
    return createError(
      res,
...
export const revokeSessionById = async (req, res) => {
  try {
    ...
    await LoggerService.logError("revokeSessionById failed", error.stack, {
      userId: req.user?.userId,
    });
    return createError(
      res,
